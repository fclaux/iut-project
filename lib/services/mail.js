...
module.exports = class MailService extends Service {
    ...
    async sendNewMovieNotification(userEmail, firstName, movieTitle) {
        ...
        try {
            const info = await this.transporter.sendMail({
                ...
                text: `Bonjour ${firstName}, le film "${movieTitle}" vient d'être ajouté à notre catalogue. Venez le découvrir !`,
                html: `<h3>Nouveau film ajouté !</h3><p>Bonjour ${firstName},</p><p>Le film <b>${movieTitle}</b> vient d'être ajouté à notre catalogue.</p><p>Venez le découvrir dès maintenant !</p>`
            });
    ...
    async sendMovieUpdateNotification(userEmail, movieTitle) {
        ...
        try {
            const info = await this.transporter.sendMail({
                ...
                text: `Bonjour, les informations concernant votre film favori "${movieTitle}" ont été modifiées.`,
                html: `<h3>Mise à jour d'un favori</h3><p>Bonjour,</p><p>Les informations concernant votre film favori <b>${movieTitle}</b> ont été modifiées par un administrateur.</p>`
            });
